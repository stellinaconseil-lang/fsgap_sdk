...
| Folder       | Content                                                                                     |
|--------------|---------------------------------------------------------------------------------------------|
| `Detection/` | Aircraft recognition. Exists today; BLOCK 4 replaces the heuristic with the live-proven rules. |
...
| `Failures/`  | `IFailureProvider` implementation mapping `FailureKey` + `FailureTarget` to Fenix failures, with an internal catalog. |
